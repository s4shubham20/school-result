                ...
                <div class="card border-top border-0 border-4 border-success">
                    ...
                    <div class="card-body">
                        <form action="{{ route('student.certificate.store') }}" class="row" method="post">
                            ...
                            @isset($jsonData)
                                @foreach ($jsonData as $key => $item)
                                    <div class="rasmussen row pe-0">
                                        ...
                                        <div class="col-md-12 mb-2">
                                            <label for="cause_of_removal{{ $key }}" class="form-label">Cause of removal</label>
                                            <input  class="form-control" id="cause_of_removal{{ $key }}" name="cause_of_removal[]" placeholder="Enter cause of removal" value="{{ $cause_of_removal[$key] ?? "" }}"/>
                                        </div>
                                    </div>
                                @endforeach
                            @endisset
                            <div id="newRow"></div>
                            <div class="my-2">
                                <button type="button" onclick="appendButton('');" class="btn mt-2 btn-info text-light"><i class="lni lni-circle-plus"></i>Add Class</button>
                            </div>
